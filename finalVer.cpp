...
private:
	void IDMapping() {
        ...
        //     fwrite(edgeArray, sizeof(int), 2, fp_NewIDEdgeFileBackup);
        //     //fp_newEdgeFileTxt << newS << " " << newD;
...
int main(){
    ...
    vector<int>edgeNum = {38,22190596, 50888414, 234370166, 1566054020, 1107805994};
    vector<int>maxVertexID = {10,1696415, 1791489, 3072441, 29452263, 22743881};
    vector<int>mergeSortMaxEdge = {2,1310720, 1310720, 2621440, 19660800, 11796480};
    for(int i = 4; i < 6; i++){
        Dataset dataset ;
        ...
        cout<<"finsh" << endl;
        tc = clock();
        fp << "use time :" << (double)(tc - start)/CLOCKS_PER_SEC << " s" << endl;
        fp << "tc finish()" <<endl;
        fp << "preprocess use time :" << (double)(preprocess - start)/CLOCKS_PER_SEC << " s" << endl;
        fp << "buildCompanionFile time :" << (double)(buildCFile - preprocess)/CLOCKS_PER_SEC << " s" << endl;
        fp << "tc time :" << (double)(tc - buildCFile)/CLOCKS_PER_SEC << " s" << endl;
