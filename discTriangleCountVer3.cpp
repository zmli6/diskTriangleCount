...
private:
	void IDMapping() {
        ...
        for (auto iter = degree2PrevID.begin(); iter != degree2PrevID.end(); iter ++) {
            ...
            pos2newID[iter->second] = idCurrent;
            //fp_oldID2newID << iter->second << " " << idCurrent << endl;
            this->index2Degree.push_back(iter->first);
            idCurrent ++;
        }
        //fp_oldID2newID.close();
        ...
        string _buf;
        //int test_count = 0;
        //ofstream fp_newEdgeFileTxt(this->newEdgeFileTxt, ios::in);
        ...
        string test = "/media/data/vend/triangleCount/doubleEdgeProject/newGraph/newGraph.txt";
        ofstream fp_test(test, ios::out);
